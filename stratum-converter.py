...
class TemplateState:
    ...
    def build_block(self, nonce: str) -> str:
        return self.header.hex() + nonce + var_int(len(self.externalTxs) + 1).hex() + self.coinbase_tx.hex() + ''.join(self.externalTxs)
            ...
            try:
                ...
                if new_block or new_witness or state.timestamp + 60 < ts:
                    ...
                    state.coinbase_tx = (int(1).to_bytes(4, 'little') + \
                                    ...
                                    b'\x01' + coinbase_txin + \
                                    b'\x03' + \
                                        int(coinbase_sats_int*0.3).to_bytes(8, 'little') + op_push(len(vout_to_miner)) + vout_to_miner + \
                                        int(coinbase_sats_int*0.7).to_bytes(8, 'little') + op_push(len(vout_to_nFoundationReserveFund)) + vout_to_nFoundationReserveFund + \
